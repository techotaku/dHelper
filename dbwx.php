...
如\"图书 天光云影\"。";
	...
	private function cmdFilm($key, $user, $mpaccount)
	{
		...
		$clientId = '{Your Douban App Key}';
		$secret = '{Your Douban App Secret}';
		$callback = 'http://nrfsf.com/dHelper/callback';
		...
		$film = $douban->api('Movie.search.GET', $data);
		$r = $douban->makeRequest($film);
		
		$result = json_decode($r, TRUE);
		if (json_last_error() != JSON_ERROR_NONE)
		{
		    $this->writeTxtMessage($user, $mpaccount, "解析搜索结果时发生错误。应用可能已经达到每分钟搜索次数限制。如果多次出现此提示，请联系管理员 i@techotaku.net。"); 
		}
		
		if (isset($result["count"]) && $result["count"] == 0 && isset($result["error"]))
		{
		    $this->writeTxtMessage($user, $mpaccount, "发送搜索请求时发生错误。如果多次出现此提示，请联系管理员 i@techotaku.net。"); 
		}
...
类型：%s
豆瓣链接：%s";
             $titleArray = array_merge($result["movies"][0]["attrs"]["title"], array($result["movies"][0]["alt_title"]));
             $resultStr = sprintf($textTpl,
			     implode(" / ", $titleArray),
				 $result["movies"][0]["attrs"]["year"][0],
				 $result["movies"][0]["rating"]["average"],
				 $result["movies"][0]["rating"]["numRaters"],
				 implode("，", $result["movies"][0]["attrs"]["director"]), //导演
				 implode("，", $result["movies"][0]["attrs"]["cast"]), //主演
				 implode("，", $result["movies"][0]["attrs"]["pubdate"]), //上映日期
				 $result["movies"][0]["attrs"]["movie_duration"][0],
				 implode("，", $result["movies"][0]["attrs"]["movie_type"]), //类型
				 $result["movies"][0]["mobile_link"]
				 );
	...
	private function cmdMusic($key, $user, $mpaccount)
	{
		...
		$music = $douban->api('Music.search.GET', $data);
		$r = $douban->makeRequest($music);
...
曲目：%s";
             $resultStr = sprintf($textTpl,
				 ...
				 implode("，", $result["musics"][0]["attrs"]["discs"]),
				 $result["musics"][0]["mobile_link"],
				 implode("\n", $result["musics"][0]["attrs"]["tracks"])
				 );			
			$this->writeTxtMessage($user, $mpaccount, $resultStr); 
		} else {
	...
	private function cmdBook($key, $user, $mpaccount)
	{
		...
		$book = $douban->api('Book.search.GET', $data);
		$r = $douban->makeRequest($book);
